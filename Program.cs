...
namespace HashCode2018
{
    ...
    public class Solution
    {
        ...
        public void PrepareDataConstraint(string fileName)
        {
            ...
            int[][] distanceBetwenRide = new int[_problem.Rides.Count][];
            int[] distanceFrom0 = new int[_problem.Rides.Count];
            for (var index = 0; index < _problem.Rides.Count; index++)
            {
                ...
                Enumerable.Range(0, _problem.Rides.Count).AsParallel().ForAll(j => distanceBetwenRide[index][j] = ride.DistanceToRide(_problem.Rides[j]));
            }

            var stringBuilder = new StringBuilder();

            stringBuilder.AppendLine($"nbTrajet={_problem.Rides.Count};");
            stringBuilder.AppendLine($"nbVehicule={_problem.Vehicles.Count};");
            stringBuilder.AppendLine($"bonus={_problem.Bonus};");
            stringBuilder.AppendLine($"nbStep={_problem.StepCount};");
            stringBuilder.AppendLine($"startStep={ArrayToString(startStep)};");
            stringBuilder.AppendLine($"endStep={ArrayToString(endStep)};");
            stringBuilder.AppendLine($"distance={ArrayToString(distance)};");
            stringBuilder.AppendLine($"distanceToNextRide={ArrayToString(distanceBetwenRide)};");
            stringBuilder.AppendLine($"distanceFrom0={ArrayToString(distanceFrom0)};");
